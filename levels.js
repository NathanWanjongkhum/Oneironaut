class Levels {
  static buildLevel(engine) {
    ...
    const worldMap = {
        ...
        lucidsunset: {
            5: this.lucidSunsetLevel1,
            6: this.lucidSunsetLevel2,
            7: this.lucidSunsetLevel3,
            8: this.lucidSunsetLevel4,
        },
        nightfall: {
            9: this.nightFallLevel1,
            10: this.nightFallLevel2,
            11: this.nightFallLevel3,
            12: this.nightFallLevel4,
        }
    };

    const selectedWorldMap = worldMap[world];
    const builderFn = selectedWorldMap?.[engine.currentLevel] || this.defaultLevel;

    builderFn.call(this, engine);

    if (!engine.sleepyGuy) engine.addEntity(new SleepyGuy(engine, 140, 120));
...
static lucidSunsetLevel1(engine) {
    const BW = PARAMS.BLOCKWIDTH;
    const gx = (c) => c * BW;
    const gy = (r) => r * BW;
  ...
  static defaultLevel(engine) { this.testLevel(engine); }
